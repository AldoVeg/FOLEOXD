...
**Incidente adicional al momento de publicar (documentado por transparencia):** al hacer `git push`, GitHub lo rechazó — el archivo "SUSTENTO ENTREGABLE 4.pdf" (275MB, el mismo archivo de prueba del usuario) había quedado comprometido dentro del historial de git en un commit local anterior a esta sesión (`0d0705b`, un "amend" hecho desde el editor), excediendo el límite de 100MB de GitHub. Se confirmó que el commit real ya publicado en `origin/main` (`1ee6dad`, con el mismo mensaje pero sin el PDF) nunca tuvo el archivo. Con autorización explícita del usuario (se le presentaron 3 alternativas: reescribir el historial local, usar Git LFS, o no publicar por ahora — eligió reescribirlo), se reconstruyó un commit limpio directamente sobre `1ee6dad` con el estado final correcto de todos los archivos (sin reescribir nada que ya estuviera publicado), se agregó un `.gitignore` para que archivos de referencia como este nunca vuelvan a versionarse por accidente, y se dejó una rama de respaldo local (`backup-pre-cleanup-20260922`) con el historial anterior por si hiciera falta. El archivo del usuario en sí **no se tocó ni se movió** — solo se dejó de rastrear por git.
